...
func (c *Client) Write() {
	...
	for {
		...
		case message, ok := <-c.Message:
			if !ok {
				_ = c.Socket.WriteMessage(websocket.CloseMessage, []byte("socket err"))
				return
			}
...
func (manager *Manager) SendService() {
	for {
		...
		case data := <-manager.Message:
			log.Printf("data: %v", string(data.Message))
...
func (manager *Manager) WsClient(ctx *gin.Context) {
	...
	log.Printf("manager.RegisterClient %v", manager.Group)
	go client.Read()
	go client.Write()
}
...
func handleMessage(c *Client, message []byte) error {
	...
	var err error
	for _, client := range WebsocketManager.Group[c.Group] {
		if c.clientId != client.clientId {
			log.Printf("当前client %v", c.clientId)
			log.Printf("发送的client %v", client.clientId)
			WebsocketManager.Send(client.clientId, c.Group, message)
		}
	}
	return err
}
